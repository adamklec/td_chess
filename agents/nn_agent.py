...
class NeuralNetworkAgent(object):
    def __init__(self, sess, trainer, name, checkpoint=None, restore=False):
        ...
        with tf.variable_scope('turn_count'):
            ...
            self.increment_turn_count_op = tf.group(game_turn_count.assign_add(1), global_turn_count.assign_add(1))
        ...
        with tf.variable_scope('update_traces'):
            for grad, var in grads_and_vars:
                ...
                with tf.variable_scope('trace'):
                    ...
                    update_accum_op = grad_accum.assign((-delta * trace) + grad_accum)
    ...
    def train(self, env, num_epochs, epsilon):
        print(self.name, "here")
        # tf.train.write_graph(self.sess.graph_def, './model/',
        #                      'td_chess.pb', as_text=False)
        # if run_name is None:
        #     summary_writer = tf.summary.FileWriter(
        #         '{0}{1}'.format('./log/', int(time.time())),
        #         graph=self.sess.graph)
        # else:
        #     summary_writer = tf.summary.FileWriter(
        #         '{0}{1}'.format('./log/', run_name), graph=self.sess.graph)

        for epoch in range(num_epochs):
            print (self.name, epoch)
