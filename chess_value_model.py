    ...
    def __init__(self):
        ...
        with tf.variable_scope('neural_network'):
            ...
            with tf.variable_scope('layer_1'):
                W_1 = tf.get_variable('W_1', initializer=tf.truncated_normal([fv_size, 200], stddev=0.001))
                hidden_1 = tf.nn.relu(tf.matmul(self.feature_vector_, W_1), name='hidden')

            with tf.variable_scope('layer_2'):
                W_2 = tf.get_variable('W_2', initializer=tf.truncated_normal([200, 100], stddev=0.001))
                hidden_2 = tf.nn.relu(tf.matmul(hidden_1, W_2), name='hidden')

            with tf.variable_scope('layer_3'):
                W_3 = tf.get_variable('W_3', initializer=tf.truncated_normal([100, 1], stddev=0.001))
